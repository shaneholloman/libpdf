...
```bash
bun scripts/create-plan.ts "$ARGUMENTS" "Your plan content here"
```
...
```bash
bun scripts/create-plan.ts "$ARGUMENTS" << HEREDOC
Your multi-line
plan content
goes here
HEREDOC
...
```bash
echo "Your content" | bun scripts/create-plan.ts "$ARGUMENTS"
```

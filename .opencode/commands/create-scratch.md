...
```bash
bun scripts/create-scratch.ts "$ARGUMENTS" "Your scratch content here"
```
...
```bash
bun scripts/create-scratch.ts "$ARGUMENTS" << HEREDOC
Your multi-line
scratch content
goes here
HEREDOC
...
```bash
echo "Your content" | bun scripts/create-scratch.ts "$ARGUMENTS"
```
